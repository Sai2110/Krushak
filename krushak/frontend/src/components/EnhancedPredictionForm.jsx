      ...
      if (user && token) {
        try {
          const predictionPayload = {
            input_data: form,
            results: data.results,  // Changed from 'predictions' to 'results' to match backend
            soil_health: data.soil_health,
            weather_data: weatherData,
            // timestamp will be set by backend
          };
          
          const saveResponse = await fetch(`${apiUrl}/api/predictions`, {
            method: 'POST',
            headers: {
          ...
          if (!saveResponse.ok) {
            console.error("Failed to save prediction to user history:", saveResult.message);
            setError(`Prediction made but failed to save: ${saveResult.message || 'Unknown error'}`);
          } else {
